    ...
    double min_fraction = 0.95)
{
    ...
    if (fraction < min_fraction) {
        RCLCPP_ERROR(logger,
                     ...
                     "trying movement waypoint recovery",
                     name.c_str(), fraction * 100.0, min_fraction * 100.0);
...
// ---------------------------------------------------------- //
   ...
   // ---------------------------------------------------------- //
        //  STEP 11 — Return to imaging state with calibration checks
        //
        //  Phase 1: Cartesian transit to perception vicinity.
        //  Phase 2: Joint snap to exact PERCEPTION_WAYPOINT angles.
        //  Phase 3: Calibration check — read back actual joint values
        //           and compare numerically against PERCEPTION_WAYPOINT.
        //           If any joint exceeds 0.01 rad (~0.57 deg), log the
        //           per-joint error and retry the snap (up to 3 times).
        //  Phase 4: Board detection — confirms camera is correctly
        //           positioned for imaging. Retries snap if not detected.
        // ---------------------------------------------------------- //
        send_feedback(gh, 1.00f, "Returning to imaging state + calibration check");
        {
            // Phase 1 — Cartesian transit to perception vicinity.
            {
            ...
            // Phase 2 + 3 — Joint snap with calibration check loop (up to 3 attempts).
            constexpr double CALIB_TOL_RAD = 0.01;   // ~0.57 degrees
            constexpr int    MAX_CALIB_ATTEMPTS = 3;
            bool calib_ok = false;

            for (int attempt = 1; attempt <= MAX_CALIB_ATTEMPTS; ++attempt) {
                // Snap to exact joint angles.
                if (!moveToJoints(move_group, PERCEPTION_WAYPOINT,
                                  "perception_waypoint_snap_" + std::to_string(attempt), logger)) {
                    RCLCPP_WARN(logger, "Joint snap attempt %d/%d failed", attempt, MAX_CALIB_ATTEMPTS);
                    continue;
                }

                // Allow controller to settle before reading back.
                rclcpp::sleep_for(std::chrono::milliseconds(200));

                // Calibration check — compare actual joints vs target numerically.
                std::vector<double> actual = move_group.getCurrentJointValues();
                calib_ok = std::inner_product(
                    actual.begin(), actual.end(), PERCEPTION_WAYPOINT.begin(), true,
                    std::logical_and<bool>(),
                    [](double a, double t){ return std::fabs(a - t) < CALIB_TOL_RAD; });

                if (!calib_ok) {
                    // Log per-joint errors so drift is visible.
                    for (size_t j = 0; j < actual.size(); ++j) {
                        double err = std::fabs(actual[j] - PERCEPTION_WAYPOINT[j]);
                        if (err >= CALIB_TOL_RAD) {
                            RCLCPP_WARN(logger,
                                "Calibration check attempt %d: joint[%zu] error=%.4f rad (%.2f deg)",
                                attempt, j, err, err * 180.0 / M_PI);
                        }
                    }
                    RCLCPP_WARN(logger, "Calibration check attempt %d/%d FAILED — retrying snap",
                                attempt, MAX_CALIB_ATTEMPTS);
                } else {
                    RCLCPP_INFO(logger, "Calibration check PASSED on attempt %d/%d — positioning accurate",
                                attempt, MAX_CALIB_ATTEMPTS);
                    break;
                }
            }
            ...
            if (!calib_ok) {
                RCLCPP_WARN(logger,
                    "Calibration check did not pass after %d attempts — continuing anyway",
                    MAX_CALIB_ATTEMPTS);
            }

            // Phase 4 — Board detection confirms camera is correctly positioned for imaging.
            {
                std::lock_guard<std::mutex> lock(board_mutex);
                board_detected = -1;
            }
...

            {
                ...
                if (board_detected != 1) {
                    ...
                    if (!moveToJoints(move_group, PERCEPTION_WAYPOINT,
                        ...
                        RCLCPP_WARN(logger, "Retry joint snap also failed — continuing anyway");
                    }
                } else {
                    RCLCPP_INFO(logger, "Board confirmed — imaging state ready");
                }
            }
        }
        // ---------------------------------------------------------- //
