...
#         print(f"[challenger] Resumed from {ch_weights_path}")
#     effective_lr = args.lr_resume if args.resume else args.lr
#     # Cosine decay: lr starts at effective_lr and decays to 5% of it over
#     # the full training run. This prevents the rising loss caused by a fixed
#     # lr overwriting learned features as the model matures.
#     total_grad_steps_estimate = args.episodes * args.updates_per_game
#     lr_schedule = tf.keras.optimizers.schedules.CosineDecay(
#         initial_learning_rate=effective_lr,
#         decay_steps=total_grad_steps_estimate,
#         alpha=0.05,   # floor = 5% of initial lr
#     )
#     optimizer = tf.keras.optimizers.Adam(learning_rate=lr_schedule, clipnorm=1.0)
#     print(f"[challenger] Optimizer lr={effective_lr} -> cosine decay over "
#           f"~{total_grad_steps_estimate:,} steps (floor={effective_lr*0.05:.2e})")
...
#                     tf.summary.scalar("training/learning_rate", current_lr, step=total_episodes)
#                     tf.summary.scalar("loss/total",   ep_total, step=total_episodes)
#                     tf.summary.scalar("loss/policy",  ep_pol,   step=total_episodes)
#                     tf.summary.scalar("loss/value",   ep_val,   step=total_episodes)
#                     tf.summary.scalar("training/grad_steps", grad_step, step=total_episodes)
...
#                     print(f"[challenger] Buffer save failed: {e}")
#                 dt = time.time() - t0
#                 current_lr = float(optimizer.learning_rate(grad_step))
#                 print(
#                     f"[challenger] ep={total_episodes:5d}  "
#                     f"az_sims={az_sims_now:3d}  "
#                     f"ply={np.mean(recent_lengths):.1f}  "
...
# if __name__ == "__main__":
#     # spawn is required for TensorFlow + multiprocessing.
#     # On macOS spawn is already the default; force=True can crash on macOS
#     # if TF has already been partially imported, so guard it.
#     try:
