...
# Penalty applied when the student pawn returns to a square it occupied
# 2 plies ago (A -> B -> A pattern). Discourages oscillation without
# blocking legitimate retreats in response to opponent walls.
# OSCILLATION_PENALTY = -0.15
OSCILLATION_PENALTY = -0.25

# Small per-step living cost applied to every non-terminal student move.
# Creates a gentle pressure to make forward progress rather than stalling.
# STEP_COST = -0.01
STEP_COST = -0.02

# Penalty when the game hits max_plies without a winner. Raised from -0.1
# so that running down the clock is clearly worse than attempting to win.
TIMEOUT_PENALTY = -0.5
...
def play_game(
    ...
    # Track recent pawn positions per side to detect A->B->A oscillation.
    # maxlen=4 keeps the last 4 positions; we only need indices -1 and -3.
    pawn_history: dict[str, deque] = {
        "bot": deque(maxlen=4),
        "player": deque(maxlen=4),
    }
    ...
    while board.game_status == "in_progress" and ply < max_plies:
        ...
        if side in record_sides and side in pending:
            ...
            # Small living cost: pressure the agent to make forward progress.
            r = STEP_COST

            # Oscillation penalty: punish returning to a square visited 2
            # plies ago (A->B->A). Only applied to the recorded (student) side
            # so the minimax opponent's positions are never penalised.
            history = pawn_history[side]
        ...
        # Record pawn position after the move for oscillation detection.
        pawn_pos = board.bot_pos_ if side == "bot" else board.player_pos_
        pawn_history[side].append((pawn_pos.x, pawn_pos.y))
