...
# # ADDED: import TensorBoard's summary writer
# from torch.utils.tensorboard import SummaryWriter  # if using PyTorch TB
# # OR use TF's built-in (no extra install needed since you already have TF):
# # writer = tf.summary.create_file_writer("./quoridor_tensorboard/")
...
#         # ADDED: track episode-level reward (sum of all transitions this game)
#         ep_reward = sum(t.reward for t in trans)
#         # ADDED: track game length
#         ep_length = len(trans)

#         ep_loss = None  # will be set below if we train this episode
#         if len(buffer) >= args.batch_size:
#             batch_losses = []
#             for _ in range(max(1, len(trans) // max(1, args.train_every))):
#                 batch = buffer.sample(args.batch_size)
#                 loss = agent.train_on_batch(batch)  # train_on_batch already returns float loss
...
#         # ADDED: write all metrics to TensorBoard every episode
...
#                 # Win rate over a rolling window of last 100 episodes
...
#                 # Training loss (only when we actually trained this episode)
...
#             writer.flush()  # make sure data is written to disk

#         # Existing checkpoint + print logic (unchanged)
#         if chief and ep % args.save_every == 0:
#             agent.save(args.model_dir)
#             dt = time.time() - t0
#             print(f"[train] ep={ep} eps={eps:.3f} buf={len(buffer)} "
