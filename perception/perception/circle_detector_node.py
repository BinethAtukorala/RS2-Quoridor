...
#         # REMOVE COMMENTS FOR NORMAL USE
#         # ---------------------------------------------------------------------------
#         # self.pipeline = rs.pipeline()
#         # self.config = rs.config()
...
class CircleDetectorNode(Node):
    ...
    def timer_callback(self):
        ...
        # for r in range(self.rows):
        #     for c in range(self.cols):
        #         if self.wall_circles[r, c] != 0 and (c, r) in sel       # inside_data = []
        ...
        # msg_inside = Float32MultiArray()
        # msg_inside.data = inside_data
        # self.pub_inside.publish(msg_inside)f.saved_coords:
        #             x, y, z = self.saved_coords[(c, r)]
        #         else:
        #             x, y, z = 0.0, 0.0, 0.0
        ...
        for wall in outside_red_walls:
            _, _, x3d, y3d, z3d = wall
            outside_data.extend([float(x3d), float(y3d), float(z3d)])
    ...
    def detect_outside_walls(self, img, depth_frame, board_corners):
        ...
        for cnt in contours:
            ...
            # Replace the depth block inside detect_outside_walls:
            if depth_frame is not None:
                depth_val = depth_frame[cy, cx]  # depth_frame is a numpy array now
                if depth_val > 0:
                    depth_m = depth_val * 0.001   # convert mm → metres (RealSense default)
                    X = (cx - self.ppx) / self.fx * depth_m
                    Y = (cy - self.ppy) / self.fy * depth_m
                    Z = depth_m
